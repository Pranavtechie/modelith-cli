...
function calculateNormalizedSimilarity(editDistance: number, maxDistance: number): number {
    // Ensure maxDistance is not zero to avoid division by zero
    if (maxDistance <= 0) { // Use <= 0 to handle potential negative distance edge cases if logic changes
        return editDistance === 0 ? 1 : 0; // If max distance is 0 or less, similarity is 1 only if edit distance is also 0
    }
    const similarity = 1 - (editDistance / maxDistance);
    return Math.max(0, Math.min(1, similarity)); // Clamp similarity between 0 and 1
}
...
/**
 * Calculates the Tree Edit Distance using the Zhang-Shasha algorithm.
 * Handles undefined nodes by returning the size of the other tree.
 * @param node1 Root of the first AST.
 * @param node2 Root of the second AST.
...
function calculateTreeEditDistance(node1: AstNode | undefined, node2: AstNode | undefined): number {
    ...
    // If one is undefined, the distance is the cost of inserting/deleting the other tree
    // Use countAstNodes as an approximation for insert/delete cost of the whole tree
    if (!node1) return countAstNodes(node2);
    if (!node2) return countAstNodes(node1);

    // Get post-order representations
    ...
    if (n === 0 && m === 0) return 0;
    if (n === 0) return m; // Cost to insert all nodes of tree 2
    if (m === 0) return n; // Cost to delete all nodes of tree 1


    // Initialize DP table for tree distances
    // treeDist[i][j] stores the TED between the subtrees rooted at
    // postOrder1[i-1] and postOrder2[j-1]
    type Row = number[]; // Define type for rows
    const treeDist: Row[] = Array(n + 1).fill(0).map(() => Array(m + 1).fill(0));

    // Initialize base cases for treeDist (deleting/inserting nodes to match empty tree)
    for (let i = 1; i <= n; i++) {
    ...
    // Iterate through all possible subtree roots in post-order
    for (let i = 1; i <= n; i++) {
        for (let j = 1; j <= m; j++) {
            // Calculate forest distance for children
            const l1 = postOrder1[i - 1]!.leftmostDescendantIndex;
            const l2 = postOrder2[j - 1]!.leftmostDescendantIndex;

            // Initialize forest distance DP table
            // fd[x][y] = distance between forest T1[l1..x] and T2[l2..y]
            // Dimensions need to cover indices from l1-1 to i and l2-1 to j
            // Size: (i - (l1-1) + 1) x (j - (l2-1) + 1) => (i - l1 + 2) x (j - l2 + 2)
            // NOTE: The previous fd table logic seemed overly complex and was removed.
            // We will recalculate the forest distance directly within the substitution cost part.

            // Option 1: Delete node i from tree 1
            const rowDel = treeDist[i - 1]!;
            const costDel = rowDel[j]! + tedCost(postOrder1[i - 1]!.node, undefined);

            // Option 2: Insert node j into tree 2
            const rowIns = treeDist[i]!;
            const costIns = rowIns[j - 1]! + tedCost(undefined, postOrder2[j - 1]!.node);

            // Option 3: Substitute (or match) node i with node j
            // This cost requires the distance between the forests of children.

            // --- Forest Distance Calculation (tempFD) ---
            // This table computes the forest distance F(i,j) needed for the treeDist substitution cost.
            // It uses previously computed treeDist values.
            // tempFD[x][y] stores the distance between T1[l1..x] and T2[l2..y]

            // Initialize the temporary forest distance DP table for *this specific* (i, j) pair
            // Indices need to go from l1-1 to i-1 and l2-1 to j-1.
            // Size: (i - (l1-1) + 1) x (j - (l2-1) + 1) => (i - l1 + 1) x (j - l2 + 1)
            // Let's adjust index mapping for clarity: dp[x'][y'] where x'=x-(l1-1), y'=y-(l2-1)
            const tempFD: number[][] = Array(i - l1 + 1).fill(0).map(() => Array(j - l2 + 1).fill(0));

            // Fill tempFD for the relevant range needed for forest F(i, j)
            // Base case: Forest T1 vs Empty Forest T2 (tempFD[x'][0])
            for (let x = l1; x <= i - 1; x++) {
                const xPrime = x - (l1 - 1);
                // Cost of deleting node x from forest T1
                tempFD[xPrime]![0] = tempFD[xPrime - 1]![0]! + tedCost(postOrder1[x - 1]!.node, undefined);
            }
            // Base case: Empty Forest T1 vs Forest T2 (tempFD[0][y']) 
            for (let y = l2; y <= j - 1; y++) {
                const yPrime = y - (l2 - 1);
                // Cost of inserting node y into forest T2
                tempFD[0]![yPrime] = tempFD[0]![yPrime - 1]! + tedCost(undefined, postOrder2[y - 1]!.node);
            }

            // Fill the rest of the tempFD table
            for (let x = l1; x <= i - 1; x++) {
                for (let y = l2; y <= j - 1; y++) {
                    const xPrime = x - (l1 - 1);
                    const yPrime = y - (l2 - 1);
                    ...
                    // Cost to delete node x from forest T1[l1..x]
                    const fdCostDel = tempFD[xPrime - 1]![yPrime]! + tedCost(postOrder1[x - 1]!.node, undefined);
                    // Cost to insert node y into forest T2[l1..y]
                    const fdCostIns = tempFD[xPrime]![yPrime - 1]! + tedCost(undefined, postOrder2[y - 1]!.node);
                    // Cost to match/substitute tree rooted at x with tree rooted at y
                    // Uses the already computed tree distance TD(x,y) = treeDist[x][y]
                    // Note: The base indices for this subproblem are l(x)-1 and l(y)-1 in the *original* treeDist table,
                    // but correspond to lxPrimeOffset-1 and lyPrimeOffset-1 in the *tempFD* table.
            ...
            // The final forest distance F(i, j) between T1[l1..i-1] and T2[l2..j-1]
            // This is the value at the bottom-right corner of tempFD: tempFD[i-1 - (l1-1)][j-1 - (l2-1)]
            const forestDistanceResult = tempFD[i - l1]![j - l2]!;
            // --- End Forest Distance Calculation ---


            const substitutionCostNode = tedCost(postOrder1[i - 1]!.node, postOrder2[j - 1]!.node);

            // Calculate treeDist[i][j] using the computed costs
            const rowSub = treeDist[l1 - 1]!;
            const subBaseCost = rowSub[l2 - 1]!;

            const currentRow = treeDist[i]!; // Re-assert non-null for the current row
    ...
    // The final TED is the distance between the full trees
    return treeDist[n]![m]!;
}

// --- End Zhang-Shasha Implementation ---
...
// const astFolderPath = './ast-files'; // Adjust path as needed
// compareAstsInFolder(astFolderPath).catch(console.error);