...
const D3HeatMap = ({
	...
	const activeCell = useRef<{x: number, y: number, value: number} | null>(null);
	const [showTooltip, setShowTooltip] = useState(false);
	const cellsRef = useRef<Map<string, {rect: SVGRectElement, j: number, i: number, value: number}>>(new Map());
    ...
    useEffect(() => {
        ...
        function getElementCellData(element: Element | null): HoveredCell {
            ...
            const cellElement = element.tagName.toLowerCase() === 'rect' 
                ? element 
			...
			<TooltipProvider>
				<Tooltip open={showTooltip}>
					...
					>
						{activeCell.current && (
							<>
								<div className="font-medium">Similarity Score: {activeCell.current.value.toFixed(4)}</div>
								<div className="text-gray-600 text-xs mt-1">
									{xLabels[activeCell.current.x]} ↔ {yLabels[activeCell.current.y]}
								</div>
							</>
						)}
					</TooltipContent>
...
export default function SimilarityMatrix() {
	...
	const search = useSearch({ from: "/similarity-matrix" });
	...
	const updateSearchParams = useCallback((params: Partial<{
		cohortId: string; 
		runId: string;
		view: "matrix" | "list";
		left: number;
	...
	const ListItem = ({ 
		...
		const colorClass = value <= leftValue 
			...
			<>
				...
				>
					<div className="flex flex-col">
						...
						<span className="text-xs text-gray-500">Click to view comparison details</span>
					</div>
				...
				<TooltipProvider>
					<Tooltip open={showListTooltip}>
						<TooltipContent 
							...
							sideOffset={5}
						>
							<div className="font-medium">Similarity Score: {value.toFixed(4)}</div>
							<div className="text-gray-600 text-xs mt-1">
								{xLabel} ↔ {yLabel}
							</div>
						</TooltipContent>
		...
		return (
			<div className="space-y-3 max-h-[800px] overflow-y-auto p-2">
				{pairs.length > 0 ? (
					...
					<div className="text-center p-6 text-gray-500">No similarity data to display</div>
				)}
			</div>
		);
	};
			...
			<div className="mb-8 bg-white p-6 rounded-lg shadow">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div>
						...
						{cohortsQuery.isLoading && <p className="mt-1 text-sm text-gray-500">Loading cohorts...</p>}
					</div>
						...
						>
							...
							{runsQuery.data?.map((run) => (
								<option key={run.runId} value={run.runId}>
									{run.name || run.runId} ({new Date(run.timestamp).toLocaleDateString()})
								</option>
							))}
						</select>
						{runsQuery.isLoading && <p className="mt-1 text-sm text-gray-500">Loading runs...</p>}
					</div>
				</div>
				
				{similarityQuery.isLoading && (
					<div className="mt-4 text-center text-gray-500">Loading similarity data...</div>
				)}
			</div>
